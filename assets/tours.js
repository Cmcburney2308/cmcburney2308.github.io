...
const photoQuips = [
    ...
    filtered.forEach(t => {
      ...
      card.innerHTML = `
        ...
        <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap">
          ...
          <button class="btn" type="button" data-remind="${t.ts}" data-city="${t.city}" data-venue="${t.venue}">Remind me</button>
        </div>
      `;
  ...
  document.addEventListener('click', (e) => {
    ...
    const city = btn.getAttribute('data-city') || 'this show';
    const venue = btn.getAttribute('data-venue') || '';
    
    const result = window.notifications.tourReminder(ts, city, venue);
